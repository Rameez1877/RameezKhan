/****** Object:  View [dbo].[Insights]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE VIEW [dbo].[Insights]
AS


WITH CTE AS
(
SELECT TagName, DisplayName,IndustryId,COUNT(1) as num 
FROM
(
SELECT DISTINCT T.Name AS tagName,RFS.IndustryId as industryId, S.DisplayName, datepart(qq, RF.PubDate) as  NewsQuarter
...
--WHERE tagName='Adam'
GROUP By  TagName, DisplayName ,IndustryId
)
--select * FROM cte
, CTE1 AS
(
...
INNER JOIN
(
	SELECT DISTINCT T.Name AS tagName, RFS.IndustryId as industryId,S.DisplayName, datepart(qq, RF.PubDate)  AS News_Quarter, COUNT(S.DisplayName) as Num
FROM            dbo.RssFeedItemSignal AS RFS
                         INNER JOIN dbo.RssFeedItem AS RF ON RFS.RssFeedItemId = RF.Id 
						 INNER JOIN dbo.Tag AS T ON RFS.TagId = T.Id and T.TagTypeId = 1
						 INNER JOIN dbo.Organization AS O ON O.Id = T.OrganizationId and O.IsActive = 1
                         ...
                         dbo.IndustryTag ON T.Id = dbo.IndustryTag.TagId AND I.Id = dbo.IndustryTag.IndustryId
						 WHERE RF.PubDate>=DATEADD(qq, DATEDIFF(qq,0,DATEADD(qq,-3,getdate())), 0) 
		GROUP By T.Name,S.DisplayName, datepart(qq, RF.PubDate),RFS.IndustryId
...
SELECT * FROM (
	select TN2.TagName, TN2.industryId ,TN2.Signal_Frequency,COUNT(TN2.DisplayName) AS NoOfSignals,
	...
	where 
		...
		from CTE1 TN2
		group by TN2.TagName, TN2.Signal_Frequency,TN2.industryId
