/****** Object:  Procedure [dbo].[GetMagazineData_old]    Committed by VersionSQL https://www.versionsql.com ******/

Create PROCEDURE [dbo].[GetMagazineData_old]
	@MagazineId int,
	@Offset int = 0,
	@Limit int = 1000
AS
...
BEGIN
	...
	Begin
	select min(Id) as Id,Link,PubDate,Title,Description,IsNull(TopicNames,'')as TopicNames,max(Score) as Score,min(SourceName) SourceName from
	(
        select RFI.Id,RFI.Link,RFI.PubDate,RFI.Title,RFI.Description,ISNULL(dbo.GetTopicNames(RFI.Id),'') as TopicNames,CS.Score,RS.Name as SourceName 
		from
