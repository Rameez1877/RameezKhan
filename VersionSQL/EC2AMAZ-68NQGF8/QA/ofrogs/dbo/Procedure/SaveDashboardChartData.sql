/****** Object:  Procedure [dbo].[SaveDashboardChartData]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[SaveDashboardChartData]
	@UserId int = 1
...
BEGIN
	...
	where o.CountryId in (select Id from #Country) and 
		o.id in
	...
	-- TABLE WITH ID
	-- CREATE TABLE DashboardSummary (OrganizationID int,UserID int,SnapshotID int)
	If not exists (select 1 from DashboardSummary where UserId = @UserId)
	Begin
		INSERT INTO DashboardSummary (OrganizationID,UserID)
	SELECT DISTINCT T1.Id , @UserId
	FROM #Result T1

	UPDATE DashboardSummary SET SnapshotID = T2.SnapshotId
	FROM DashboardSummary T1
	INNER JOIN VendorUploadedProfile T2
