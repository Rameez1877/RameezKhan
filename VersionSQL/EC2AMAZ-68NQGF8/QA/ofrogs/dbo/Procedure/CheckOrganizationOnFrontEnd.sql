/****** Object:  Procedure [dbo].[CheckOrganizationOnFrontEnd]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[CheckOrganizationOnFrontEnd]
    @UserId int = 0,
...
BEGIN
    ...
    FROM AppUser
    WHERE Id = @UserId
    Declare @Limit int = IIF(@AppRoleID = 3, 50, 1000000)
  ...
  --          FROM dbo.Split(@Segment, ',')))
    ...
    IF @TechnTagids = '' AND @GicCountryIds = '' AND @Segment = '' AND @Coe = ''
	BEGIN
    ...
    IF @TechnTagids <> '' OR @GicCountryIds <> '' OR @Segment <> '' OR @Coe <> ''
    BEGIN
        WITH
            cte
            AS
