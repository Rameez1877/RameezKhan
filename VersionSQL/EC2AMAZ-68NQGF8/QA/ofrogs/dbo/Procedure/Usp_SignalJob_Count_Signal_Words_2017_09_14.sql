/****** Object:  Procedure [dbo].[Usp_SignalJob_Count_Signal_Words_2017_09_14]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[Usp_SignalJob_Count_Signal_Words_2017_09_14]
@TagId int ,
@IndustryId int,
@SignalId int
	...
	Create Table #TempModel (rssfeeditemId INT,SignalId INT, rssTypeId INT, signalKeyWord varchar(MAX),  IndustryID int, TagID int
	 ...
	 insert into #TempModel
	  select Id, SignalID, RssTypeID, SignalKeyWord,  IndustryID, TagID from (
		select  RF.id as Id, SW.SignalId as SignalId, ADN.RssTypeID as RssTypeID,	
				SW.SignalKeyWord as SignalKeyWord,
				
				Case
					when  dbo.[fn_GetOrgName_Title](RF.title,' ',SW.SignalKeyWord) =1  then 1 
					when len(SW.SignalKeyWord) > 4 and RF.title Collate SQL_Latin1_General_CP1_CI_AS like '%' + SW.SignalKeyWord + '%' then 0.6
					 --when len(SW.SignalKeyWord) <5  and dbo.[fn_GetOrgName_Title](RF.title,' ',SW.SignalKeyWord) =1  then 1
				     when len(SW.SignalKeyWord) <5  and dbo.fn_remove_Chars(RF.title) Collate SQL_Latin1_General_CP1_CI_AS like '%' + SW.SignalKeyWord + '^%'  then 1
		...
		where  RF.Title collate SQL_Latin1_General_CP1_CI_AS like '%' + SW.SignalKeyWord + '%' and 
			ADN.rssfeeditemid = RF.id 
			and SW.signalID not in (11,34,33)
			and ADN.IndustryID = SW.IndustryID
			and ADN.RssTypeID = SW.SignalRssType
			UNION
			
			select  RF.id, SW.SignalId, ADN.RssTypeID, SW.SignalKeyWord,
			
				Case	
						when dbo.[fn_GetOrgName_Title](RF.news,' ',SW.SignalKeyWord) =1  then 1.0
						when len(SW.SignalKeyWord) > 4 and RF.news like '%' + SW.SignalKeyWord + '%' then 0.9	
					
						 when len(SW.SignalKeyWord) <5 and RF.news like '%' + SW.SignalKeyWord + '^%'  then 0.9
						 when len(SW.SignalKeyWord) <5 and RF.news like '%' + SW.SignalKeyWord + '%'  then 0.6
		  ...
		  @SignalKeyWords SW, rssfeeditem RF,  @DomainNews ADN
		   where 
			RF.News collate SQL_Latin1_General_CP1_CI_AS like '%' + SW.SignalKeyWord + '%' and 
			ADN.rssfeeditemid = RF.id 
			and SW.signalID not in (11,34,33)
			and ADN.IndustryID = SW.IndustryID
			and ADN.RssTypeID = SW.SignalRssType
