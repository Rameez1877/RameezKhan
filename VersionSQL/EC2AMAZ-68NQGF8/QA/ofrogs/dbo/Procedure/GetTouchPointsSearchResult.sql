/****** Object:  Procedure [dbo].[GetTouchPointsSearchResult]    Committed by VersionSQL https://www.versionsql.com ******/

-- Stored Procedure

-- JAN-2022 - KABIR 
-- EXEC [QA_GetTouchPointsSearchResult] 159, 1
CREATE PROCEDURE [dbo].[GetTouchPointsSearchResult]  
 @UserID int,
@Page INT = 0,
@Size INT = 10,
...
BEGIN
--set @UserID = 159
DECLARE @All BIT, @Count INT ,@F VARCHAR(200),  @AppRoleID int
...
SELECT @Count = COUNT(*) 
	...
	WHERE 
			...
			AND C.USERID = @USERID and
			 ...
			 Url,
			 Country ,
			 IIF(@AppRoleID = 3 AND @Page > 1, '*', EmailId) as EmailId,
			 WebsiteUrl,
			...
			--(@Designation = '' OR designation LIKE '%' + @Designation + '%')
			 (@CountryName = '' OR Country = @CountryName)
