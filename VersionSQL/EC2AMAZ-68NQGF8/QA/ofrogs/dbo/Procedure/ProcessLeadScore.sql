/****** Object:  Procedure [dbo].[ProcessLeadScore]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[ProcessLeadScore]
...
BEGIN
  ...
  DECLARE @Query nvarchar(4000)
	...
	select CountryId AS Id into #Countries from TargetPersonaCountry where TargetPersonaId = @TargetPersonaId
	...
	from 
		...
		inner join UserTargetFunctionality utf on (mc.[Name] = utf.Functionality and utf.TargetPersonaId = @TargetPersonaId)
	where
	...
	If exists (select 1 from UserTargetIndustry where TargetPersonaId = @TargetPersonaId and ApplyScore = 1)
	Begin
		...
		from
			#Org o
			left join UserTargetIndustry uti on (o.IndustryId = uti.IndustryId and uti.TargetPersonaId = @TargetPersonaId)
	...
	If exists (select 1 from UserTargetTechnology where TargetPersonaId = @TargetPersonaId and ApplyScore = 1)
    Begin
		;with cte as
		...
		from
			...
			inner join UserTargetTechnology utt on (utt.TargetPersonaId = @TargetPersonaId and s.Keyword = utt.Technology)
        group by o.Id
        )
	...
	Begin
		...
		from
			...
			inner join UserTargetTechnology utf on (utf.TargetPersonaId = @TargetPersonaId and s.Technology = utf.Technology)
		where
    ...
    If exists (select 1 from UserTargetFunctionality where TargetPersonaId = @TargetPersonaId and ApplyScore = 1)
    BEGIN
        ...
        from
            ...
            inner join UserTargetFunctionality utf on (utf.TargetPersonaId = @TargetPersonaId and utf.Functionality = ot.TeamName)
    ...
    BEGIN
		...
		from 
			#Org o 
			inner join #Surge s on (o.Id = s.OrganizationId)
			inner join UserTargetFunctionality utf on (utf.TargetPersonaId = @TargetPersonaId and s.Functionality = utf.Functionality)
    ...
    BEGIN
		...
		from
			#Org o
			inner join #Surge s on (o.Id = s.OrganizationId)
			inner join UserTargetFunctionality utf on (utf.TargetPersonaId = @TargetPersonaId and utf.Functionality = s.Functionality)
