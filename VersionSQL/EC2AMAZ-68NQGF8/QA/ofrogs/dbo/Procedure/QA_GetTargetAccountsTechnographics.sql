/****** Object:  Procedure [dbo].[QA_GetTargetAccountsTechnographics]    Committed by VersionSQL https://www.versionsql.com ******/

-- Script Date: 18-Nov-21  - Kabir
CREATE PROCEDURE [dbo].[QA_GetTargetAccountsTechnographics] @targetpersonaid int
AS
BEGIN
  ...
  SET @Limit = (SELECT IIF(@AppRoleId = 3 , 200,5000))

    SET @Cond = IIF(@Type = 'App' ,1,@Cond)
  SET @Cond = IIF(@Type <> 'App' AND @Gics <> '' ,2,@Cond)
  SET @Cond = IIF(@Type <> 'App' AND @Gics = '' AND (@PERSONA = '' OR @PERSONA IS  NULL) ,3,@Cond)
  SET @Cond = IIF(@Type <> 'App' AND @Gics = '' AND (@PERSONA <> '' AND @PERSONA IS NOT NULL),4,@Cond)
...
--         EmployeeCount,
SELECT DISTINCT
	O.ID OrganizationId,O.NAME OrganizationName, T.Keyword Technology,T.SubCategory TechnologyCategory,C.name CountryName,
    ...
    BEGIN 
	  ...
	  PRINT '3'
        SELECT DISTINCT TOP (@Limit)
          ...
          o.name AS OrganizationName,
          Tech.Keyword AS Technology,
          tst.SubCategory AS TechnologyCategory,
          c.name AS CountryName,
          i.name AS IndustryName,
          o.Revenue AS Revenue,
          o.EmployeeCount AS EmployeeCount,
        ...
        INNER JOIN #INDUSTRYID I
		  ...
		  where  (@TechnologyCategory = ''
          OR tst.SubCategory IN (SELECT
            value
          FROM STRING_SPLIT(@TechnologyCategory, ','))
          )
		  AND (@Industry = ''
