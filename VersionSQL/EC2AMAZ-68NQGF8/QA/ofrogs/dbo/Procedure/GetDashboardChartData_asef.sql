/****** Object:  Procedure [dbo].[GetDashboardChartData_asef]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE  PROCEDURE [dbo].[GetDashboardChartData_asef]
	 @UserId int = 159
AS 
BEGIN
SET NOCOUNT ON
	declare @PersonaIds varchar(100), @RegionIds varchar(100), @CustomerType varchar(15), @RevenueCategoryIds varchar(15), @IndustryGroupIds varchar(100)

	SELECT @PersonaIds = PersonaIds, @RegionIds = RegionIds, @CustomerType = CustomerType,
	@RevenueCategoryIds = RevenueCategoryIds, @IndustryGroupIds = IndustryGroupIds
	...
	and o.IndustryId  in (select Id from #industry)
	---- end ---
	and	o.id in
...
--		UserID INT)
	...
	-- changed by Asef on 28 oct 2021
	-- replaced Category with Revenue Name
	--select top 4 RM.Revenue as [Name], count(DISTINCT (R.Id)) as [Value] 
	--from #Result R 
	--INNER JOIN RevenueMapping RM ON R.Revenue = RM.Revenue 
	--INNER JOIN REVENUECATEGORY RC ON RC.NAME = RM.CATEGORY
	--INNER JOIN AppUser A ON (RC.ID IN (SELECT VALUE FROM string_split(A.RevenueCategoryIds, ',') WHERE A.id= @UserId))
	--where R.Revenue is not null 
	--group by RM.Revenue order by 2 desc
	
	-- new changes for employee count

	--select top 4 EM.EmployeeCount as [Name], count(DISTINCT (R.Id)) as [Value] 
	--from #Result R 
	--INNER JOIN EmployeeCountMapping EM ON R.EmployeeCount = EM.EmployeeCount 
	--INNER JOIN REVENUECATEGORY RC ON RC.NAME = EM.CATEGORY
	--INNER JOIN AppUser A ON (RC.ID IN (SELECT VALUE FROM string_split(A.RevenueCategoryIds, ',') WHERE A.id= @UserId))
	--where R.EmployeeCount is not null 
	--group by EM.EmployeeCount order by 2 desc
...
--IndustryGroupIds	RevenueCategoryIds
--EmployeeCountMapping -- RevenueMapping -- RevenueCategory
...
--SELECT COUNT(DISTINCT ID) FROM #Result -- RABBIT CHECK
--WHERE Revenue IN (
--'<1M'
--,'10M-50M'
--,'50M-100M'
...
--SELECT COUNT(DISTINCT ID) FROM #Result -- DEER CHECK
--WHERE Revenue IN (
--'100M-250M'
--,'250M-500M'
--,'500M-1B')
