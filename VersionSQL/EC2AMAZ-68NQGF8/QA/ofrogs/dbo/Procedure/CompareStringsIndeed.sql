/****** Object:  Procedure [dbo].[CompareStringsIndeed]    Committed by VersionSQL https://www.versionsql.com ******/

Create Procedure [dbo].[CompareStringsIndeed]
@id int
AS
BEGIN
	SET NOCOUNT ON;
	...
	select @Title = CompanyName from IndeedJobPost where id=@id
	...
	if(@Score>0.8)
	 update IndeedJobPost set tagid=@TagId where id=@id
	 else
	  update IndeedJobPost set tagid=0 where id=@id 
END
