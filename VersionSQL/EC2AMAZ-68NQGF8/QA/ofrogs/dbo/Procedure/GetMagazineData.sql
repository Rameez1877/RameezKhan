/****** Object:  Procedure [dbo].[GetMagazineData]    Committed by VersionSQL https://www.versionsql.com ******/

Create PROCEDURE [dbo].[GetMagazineData] --  2147  
 @MagazineId int,      
 @Offset int = 0,      
 @Limit int = 1000      
AS      
...
(      
 ...
 Begin      
  ...
  from(select Top (@Limit)I.Id, I.Link, I.PubDate, I.Title, I.[Description], Tag.Name, ISNULL(dbo.[GetSignalNames](I.Id),'') as [TopicNames],0.3 as Score, RS.Name as [SourceName] from dbo.RssFeedItem I inner join dbo.RssFeedItemTag T on (T.RssFeedItemId = I.Id)  inner join Tag on t.tagid = Tag.id inner join dbo.RssFeed F on (F.Id = I.RssFeedId) inner join dbo.RssSource RS on (RS.Id = F.RssSourceId)      
