/****** Object:  Procedure [dbo].[SearchOrganization_new]    Committed by VersionSQL https://www.versionsql.com ******/

--exec SearchOrganization_new 1,'','','125','','','AWS','','',''
CREATE PROCEDURE [dbo].[SearchOrganization_new]
     @UserId int = 0,
...
BEGIN
    ...
    END
			...
			SELECT DISTINCT top (@Limit)
				...
				D.[NAME],
				D.WebsiteUrl,
				SUBSTRING (D.WebsiteUrl, CHARINDEX( '.', D.WebsiteUrl) + 1,
				LEN(D.WebsiteUrl)) AS [Domain],
				D.EmployeeCount,
				D.Revenue,
				D.IndustryID,
				D.IndustryName,
			...
			WHERE UserID = @UserId
            AND (@IndustryIds = ''
            OR IndustryId IN (SELECT
                value
            FROM STRING_SPLIT(@IndustryIds, ',')) )
            AND (@CountryIds = ''
            OR CountryId IN (SELECT
                value
            FROM STRING_SPLIT(@CountryIds, ',')) )
            AND (@EmployeeCounts = ''
			...
			AND (@TechnTagids = ''
            OR Technology IN (SELECT
                value
            FROM STRING_SPLIT(@TechnTagids, ',')) )
			AND (@GicCountryIds = ''
            OR Id IN (SELECT OrganizationId from #tempGIC))
			AND (@Functionalities = ''
